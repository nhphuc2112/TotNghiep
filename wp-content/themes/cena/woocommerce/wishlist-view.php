    ...
    <table class="shop_table cart wishlist_table wishlist_view traditional responsive" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr( $wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
        ...
        <tr>
            ...
            <th class="product-name">
                <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product Name', 'cena' ) ) ?></span>
            </th>
            ...
            <?php if( $show_price ) : ?>
                ...
                <th class="product-price">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit Price', 'cena' ) ) ?>
                    </span>
                </th>
            ...
            <?php if( $show_stock_status ) : ?>
                ...
                <th class="product-stock-status">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'cena' ) ) ?>
                    </span>
                </th>
            ...
            <?php if( $show_last_column ) : ?>
                ...
                <th class="product-add-to-cart"><?php esc_html_e( 'Add to cart', 'cena' ); ?></th>
            ...
            <?php if( $is_user_owner ): ?>
                <th class="product-remove"><?php esc_html_e( 'Remove', 'cena' ); ?></th>
            <?php
                $column_count ++;
            endif;
            ?>
        ...
        if( count( $wishlist_items ) > 0 ) :
                        ...
                        <tr id="yith-wcwl-row-<?php echo esc_attr($item['prod_id']); ?>" data-row-id="<?php echo esc_attr($item['prod_id']); ?>">
                            ...
                            <?php if( $show_price ) : ?>
                                <td class="product-price">
                                    <?php 
                                        $html_price = ($product->get_price()) ? $product->get_price_html() : apply_filters( 'yith_free_text', esc_html__( 'Free!', 'cena' ) );
                                    ?>
                            ...
                            <?php if( $show_stock_status ) : ?>
                                <td class="product-stock-status">
                                    ...
                                    <?php 
                                        $html_stock = ($stock_status == 'out-of-stock') ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'cena' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'cena' ) . '</span>';
                                    ?>
                            ...
                            <td class="product-add-to-cart">
                                ...
                                if( $show_dateadded && isset( $item['dateadded'] ) ):
                                    echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on : %s', 'cena' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
                                endif;
                                ?>
                                ...
                                <select class="change-wishlist selectBox">
                                    <option value=""><?php esc_html_e( 'Move', 'cena' ) ?></option>
                                    <?php
                                    ...
                                    ?>
                                        <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
                                            ...
                                            if( $wl['wishlist_privacy'] == 1 ){
                                                $wl_privacy = esc_html__( 'Shared', 'cena' );
                                            }
                                            elseif( $wl['wishlist_privacy'] == 2 ){
                                                $wl_privacy = esc_html__( 'Private', 'cena' );
                                            }
                                            else{
                                                $wl_privacy = esc_html__( 'Public', 'cena' );
                                            }
                            ...
                            <td class="product-remove">
                                <div>
                                    <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php esc_attr_e( 'Remove this product', 'cena' ) ?>"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                            ...
                            <?php if ($repeat_remove_button) : ?>
                                <td class="product-remove">
                                    <div>
                                        <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php esc_attr_e( 'Remove this product', 'cena' ) ?>"><?php esc_html_e( 'Remove', 'cena' ) ?></a>
                                    </div>
                                </td>
                            <?php endif; ?>
                        </tr>
            ...
            <tr>
                <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products were added to the wishlist', 'cena' ) ) ?></td>
            </tr>
        <?php
        endif;
