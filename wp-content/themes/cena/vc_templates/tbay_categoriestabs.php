...
cena_tbay_get_category_childs( $categories, 0, 0, $cat_array );
        ...
        <div class="widget-content woocommerce">
            <ul role="tablist" class="product-categories-tabs-title tabs-list nav nav-tabs" <?php echo trim($tabs_data); ?>>
                <?php foreach ($categoriestabs as $tab) : ?>
                    ...
                    if( !in_array($tab['category'], $cat_array_id) ) {
                        $cat_category    = esc_html__('all-categories','cena');
                        $cat_name        = esc_html__('All Categories','cena');
                        $slug            = '';
                    } else {
                        $cat_category    = $tab['category'];
                        $category        = get_term_by( 'id', $cat_category, 'product_cat' );
                        $cat_name        = $category->name;
                        $slug            = cena_get_transliterate($category->slug);
                    }
                    ...
                    <li <?php echo trim($class_li); ?>>
                       <a href="#tab-<?php echo esc_attr($_id);?>-<?php echo esc_attr($i); ?>" data-toggle="tab"  data-value="<?php echo esc_attr($slug); ?>" class="tbay-image-loaded">
                            <?php if ( isset($tab['icon']) && !empty($tab['icon']) ): ?>
                                <?php $img = wp_get_attachment_image_src($tab['icon'], 'full'); ?>
                                <?php if ( isset($img[0]) ) { ?>
                                    <?php 
                                        cena_tbay_src_image_loaded($img[0], array('alt' => $title)); 
                                    ?>
                                <?php } ?>
            ...
            <div class="widget-inner">
                <?php if( !empty($image_cat) ) : ?>
                    ...
                    <div class="col-lg-3 hidden-md hidden-sm hidden-xs tbay-image-loaded <?php echo esc_attr( $image_float );?>">
                        <?php 
                            $image_alt  = get_post_meta( $image_cat, '_wp_attachment_image_alt', true);
                            cena_tbay_src_image_loaded($img[0], array('alt' => $image_alt)); 
                        ?>
                    </div>
                <?php endif; ?>
                <div class="<?php echo !empty($image_cat) ? 'col-lg-9 col-xs-12' : ''; ?>">
                    <div class="tab-content tbay-addon-content">
                        <?php $i = 0; foreach ($categoriestabs as $tab) : ?>
                            
                            <?php 
                             $tab_active = ($i == 0) ? ' active active-content current' : '';
                            ?>

                            <div id="tab-<?php echo esc_attr($_id);?>-<?php echo esc_attr($i); ?>" class="tab-pane <?php echo esc_attr( $tab_active ); ?>">
                                <?php if( $i === 0 || $ajax_tabs !== 'yes' ) : ?>

                                <?php 
                                     if( !in_array($tab['category'], $cat_array_id) ) {
                                        $cat_category    = esc_html('all-categories','cena');
                                        $loop      	     = cena_get_query_products('', $cat_operator, $type, $number);
                                        $link            = get_permalink( wc_get_page_id( 'shop' ) );
                                    } else {
                                        $category   = get_term_by( 'id', $tab['category'], 'product_cat' );
                                        $cat_category = $category->slug;
                                        $loop      	    = cena_get_query_products($cat_category, $cat_operator, $type, $number);
                                ...
                                <?php endif; ?>
                                <a href="<?php echo esc_url( $link ); ?>" class="btn btn-block btn-view-all"><?php echo esc_html__('view all', 'cena'); ?><i class="fa fa-icon-long-arrow-right"></i></a>
