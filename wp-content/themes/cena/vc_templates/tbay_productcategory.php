...
if ( isset($category) || !empty($category) ):
	$loop = cena_get_query_products( $category, $cat_operator, $product_type, $number);
...
if( !in_array($category, $cat_array_id) ) {
	$category = -1;
    $loop            = cena_get_query_products('', $cat_operator, $product_type, $number);
} else {
	$cat_category = get_term_by( 'id', $category, 'product_cat' );
	$slug 		  = $cat_category->slug;
	$loop 		  = cena_get_query_products($slug, $cat_operator, $product_type, $number);
}
