<?php 
    $tbay_header = apply_filters( 'cena_tbay_get_header_layout', cena_tbay_get_config('header_type') );
    ...
    $menu_attribute         = '';
    $menu_title             = cena_tbay_get_config('menu_mobile_title', 'Menu mobile');


    if( $menu_option == 'smart_menu' ) {
        $search_items           = cena_tbay_get_config('menu_mobile_search_items', 'Search menu item');
        $search_no_results      = cena_tbay_get_config('menu_mobile_no_esults', 'No results found.');
        $search_splash          = cena_tbay_get_config('menu_mobile_search_splash', 'What are you looking for? Start typing to search the menu.');
        $search_enable          = cena_tbay_get_config('enable_menu_mobile_search', false);
        $menu_counters          = cena_tbay_get_config('enable_menu_mobile_counters', false);  

        $menu_second            = cena_tbay_get_config('enable_menu_second', false);  
 




        $menu_mobile_themes            = cena_tbay_get_config('menu_mobile_themes', 'light');
        $menu_attribute                .= 'data-themes="' . $menu_mobile_themes . '" ';  

        /*Socials*/
        $enable_menu_social           = cena_tbay_get_config('enable_menu_social', false); 
        $menu_attribute           .= 'data-enablesocial="' . $enable_menu_social . '" '; 
        if( $enable_menu_social ) {
            $social_slides            = cena_tbay_get_config('menu_social_slides');  
        ...
        if( $menu_second ) {
            ...
            $menu_second_id         =  cena_tbay_get_config('menu_mobile_second_select');

            $menu_tab_one           = cena_tbay_get_config('menu_mobile_tab_one', 'Menu');
            $menu_tab_one_icon      = cena_tbay_get_config('menu_mobile_tab_one_icon', 'fa fa-bars');            

            $menu_tab_second        = cena_tbay_get_config('menu_mobile_tab_scond', 'Categories');
            $menu_tab_second_icon   = cena_tbay_get_config('menu_mobile_tab_second_icon', 'fa fa-th');
        ...
        /*Effect */
        $enable_effects            = cena_tbay_get_config('enable_menu_mobile_effects', false);  
        $menu_attribute           .= 'data-enableeffects="' . $enable_effects . '" '; 

        if($enable_effects) {
            $effects_panels        =  cena_tbay_get_config('menu_mobile_effects_panels', '');
            $effects_listitems     =  cena_tbay_get_config('menu_mobile_effects_listitems', '');
    ...
    $menu_one_id    =  cena_tbay_get_config('menu_mobile_one_select');
    ...
    <div class="tbay-offcanvas-body"> 
        ...
        <nav id="tbay-mobile-menu-navbar" class="menu navbar navbar-offcanvas-mmneu navbar-static">
            <?php
                ...
                $args['walker']             =   new Cena_Tbay_mmenu_menu();
                ...
                if( isset($menu_second) && $menu_second ) {
                    ...
                    if( !empty($menu_second_id) ) {
                        ...
                        $menu_second_name = cena_get_transliterate($menu_second_obj->slug);
                    }  
                    ...
                    $args_second['walker']             =   new Cena_Tbay_mmenu_menu();
