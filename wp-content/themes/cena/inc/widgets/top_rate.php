...
if (class_exists('PostRatings')) {
    ...
    class Tbay_Top_Rate_Widget extends Tbay_Widget {
        public function __construct() {
            parent::__construct(
                // Base ID of your widget
                'tbay_top_rate_widget',
                // Widget name will appear in UI
                esc_html__('Tbay Top Rate Widget', 'cena'),
                // Widget description
                array( 'description' => esc_html__( 'The highest rated posts on your site', 'cena' ), )
            );
        ...
        public function form( $instance ) {
            ...
            ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e('Title:', 'cena') ?></label>
                <br>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php esc_html_e('Get most rated:', 'cena'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('sort')); ?>"><?php esc_html_e('Sort by:', 'cena'); ?></label>
                <select class="widefat" id="<?php echo esc_attr($this->get_field_id('sort')); ?>" name="<?php echo esc_attr($this->get_field_name('sort')); ?>">
                <option <?php selected('bayesian_rating', $instance['sort']); ?> value="bayesian_rating"><?php esc_html_e('Overall bayesian rating (score)', 'cena'); ?></option>
                <option <?php selected('rating', $instance['sort']); ?> value="rating"><?php esc_html_e('Average rating', 'cena'); ?></option>
                <option <?php selected('votes', $instance['sort']); ?> value="votes"><?php esc_html_e('Number of votes', 'cena'); ?></option>
                </select>
            </p>
            ...
            <p>
                ...
                <?php esc_html_e('Descending', 'cena'); ?>
                </label>
                ...
                <?php esc_html_e('Ascending', 'cena'); ?>
                </label>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('date_limit')); ?>"><?php esc_html_e('Ignore posts older than:', 'cena'); ?></label>
                <input id="<?php echo esc_attr($this->get_field_id('date_limit')); ?>" name="<?php echo esc_attr($this->get_field_name('date_limit')); ?>" type="text" value="<?php echo esc_attr( $instance['date_limit'] ); ?>" size="3" /> <?php esc_html_e('days', 'cena'); ?>
                <br />
                <small><?php esc_html_e('(0 to ignore date)', 'cena'); ?></small>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Limit:', 'cena'); ?></label>
