...
if ( !class_exists("Cena_Tbay_Custom_Nav_Menu") ) {
    class Cena_Tbay_Custom_Nav_Menu extends Walker_Nav_Menu {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( isset($item->tbay_text_label) && !empty($item->tbay_text_label) ) {
                switch ( $item->tbay_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'cena');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'cena');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'cena');
                        break;
        ...
        public function generateSubMegaMenu( $item, $tbay_mega_profile ) {
            ...
            if ( $tbay_mega_profile ) {
                ...
                if ( $posts && isset($posts[0]) ) {
                    ...
                    if( cena_tbay_get_config('ajax_dropdown_megamenu', false) ) {  
