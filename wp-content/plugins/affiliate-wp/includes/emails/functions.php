...
function affwp_get_monthly_email_summary_content( $timeframe ) {
	...
	$intro  = __( 'Xin chào!', 'affiliate-wp' );
	$intro2 = __( "Hãy xem lại hoạt động của bạn trong 30 ngày vừa qua.", 'affiliate-wp' );
	...
	if (
		...
		<?php if ( $has_active_integration_supports_sales_repoorting ) : // Only if there is an integration active that supports sales. ?>
			...
			<?php esc_html_e( 'Tổng doanh thu', 'affiliate-wp' ); ?>: <?php echo esc_html( $total_program_revenue ); ?>
		<?php endif; ?>

		<?php esc_html_e( 'Liên kết mới được chấp nhận', 'affiliate-wp' ); ?>: <?php echo absint( $new_approved_affiliates ); ?>

		<?php echo esc_html( __( 'Doanh thu chưa được thanh toán', 'affiliate-wp' ) ); ?>: <?php echo esc_html( $unpaid_earnings ); ?>

		<?php echo esc_html( __( 'Doanh thu đã được thanh toán', 'affiliate-wp' ) ); ?>: <?php echo esc_html( $paid_earnings ); ?>

		<?php if ( ! empty( $top_affiliates ) ) : ?>

			<?php echo esc_html( sprintf( __( 'Top %1$s liên kết có doanh thu cao nhất', 'affiliate-wp' ), count( $top_affiliates ) ) ); ?>:
			<?php foreach ( $top_affiliates as $top_affiliate_id ) : ?>
				...
				<?php esc_html_e( 'Số doanh thu đạt được', 'affiliate-wp' ); ?>: <?php echo esc_html( affwp_currency_filter( affwp_format_amount( affiliate_wp()->referrals->get_earnings_by_status( array( 'paid', 'unpaid' ), $top_affiliate_id, $timeframe ) ) ) ); ?>

				<?php esc_html_e( 'Số doanh thu từ giới thiệu', 'affiliate-wp' ); ?>: <?php echo esc_html( affiliate_wp()->referrals->count_by_status( array( 'paid', 'unpaid' ), $top_affiliate_id, $timeframe ) ); ?>

				<?php esc_html_e( 'Lượt ghé thăm', 'affiliate-wp' ); ?>: <?php echo esc_html( affwp_count_visits( $top_affiliate_id, $timeframe ) ); ?>
		...
		<div class="data" style="margin: 30px 0; width: 100%;">
			<div style="text-align: center;">
					...
					<div style="height: 150px; width: 250px; display: inline-table;">
						...
						<p style="margin-bottom: 0; margin-top: 5px; font-size: 14px; line-height: 18px;">
							<strong style="font-weight: 500;">
								<?php esc_html_e( 'Tổng doanh thu kiếm được', 'affiliate-wp' ); ?>
							</strong>
						</p>
				...
				<div style="height: 150px; width: <?php echo esc_attr( $has_active_integration_supports_sales_repoorting ? '250px' : '160px' ); ?>; display: inline-table;">
					...
					<p style="margin-bottom: 0; margin-top: 5px; font-size: 14px; line-height: 18px;">
						<strong style="font-weight: 500;">
							<?php echo wp_kses_post( __( 'Liên kết mới được chấp nhận', 'affiliate-wp' ) ); ?>
						</strong>
					</p>
				...
				<div style="height: 150px; width: <?php echo esc_attr( $has_active_integration_supports_sales_repoorting ? '250px' : '160px' ); ?>; display: inline-table;">
					...
					<p style="margin-bottom: 0; margin-top: 5px; font-size: 14px; line-height: 18px;">
						<strong style="font-weight: 500;">
							<?php echo wp_kses_post( __( 'Doanh thu chưa được thanh toán', 'affiliate-wp' ) ); ?>
						</strong>
					</p>
				...
				<div style="height: 150px; width: <?php echo esc_attr( $has_active_integration_supports_sales_repoorting ? '250px' : '160px' ); ?>; display: inline-table;">
					...
					<p style="margin-bottom: 0; margin-top: 5px; font-size: 14px; line-height: 18px;">
						<strong style="font-weight: 500;">
							<?php echo wp_kses_post( __( 'Doanh thu đã được thanh toán', 'affiliate-wp' ) ); ?>
						</strong>
					</p>
	...
	<?php if ( ! empty( $top_affiliates ) ) : ?>
		<div style="border-top: 1px solid #eee; padding-top: 20px; margin-bottom: 40px;">
			...
			<table style="width: 100%; border-collapse: collapse; font-size: 12px; line-height: 12px; color: #000000;" cellpadding="10">
				<tbody>
					<tr>
						<td><strong style="font-weight: 500;"><?php esc_html_e( 'Affiliates', 'affiliate-wp' ); ?></strong></td>
						<td><strong style="font-weight: 500;"><?php esc_html_e( 'Doanh thu', 'affiliate-wp' ); ?></strong></td>
						<td><strong style="font-weight: 500;"><?php esc_html_e( 'Giới thiệu', 'affiliate-wp' ); ?></strong></td>
						<td><strong style="font-weight: 500;"><?php esc_html_e( 'Lượt ghé', 'affiliate-wp' ); ?></strong></td>
					</tr>
