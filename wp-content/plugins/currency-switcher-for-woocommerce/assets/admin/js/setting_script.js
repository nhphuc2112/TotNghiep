jQuery(document).ready(function ($) {
    ...
    jQuery( document ).on( 'click', '.wccs_zp_edit', function(e) {
        ...
        var zp_prefix = jQuery(parentel).find( '.zp_prefix' ).text();
        ...
        jQuery( '#wccs_zp_prefix' ).val( zp_prefix ).trigger( 'change' );
    ...
    jQuery( '#wccs_add_zp' ).on( 'click', function(e) {
        ...
        var zp_prefix = jQuery.trim( jQuery('#wccs_zp_prefix').val() );
            ...
            jQuery.ajax({
                ...
                success: function( response ) {                    
                    if ( response != '' ) {
                        ...
                        jQuery('#wccs_zp_prefix').val('').trigger('change');
    ...
    $(document).on("click", ".wccs_remove_currency", function(){
        ...
        if($('#wccs_currencies_table tbody tr').length == 0){
            $('#wccs_currencies_table tbody tr').hide();
        }
    });
            ...
            if (response.status) {
                ...
                for(var k in data) {
                    if(data.hasOwnProperty(k)) {
                        formattedNumber = data[k].replace(/,/g, "");
                        $('input[name="wccs_currencies['+k+'][rate]"]').val(formattedNumber);
                    }
                }
    ...
    $(document).on("click", ".wccs_update_rate", function(){
        ...
        $.post(variables.ajaxurl, data, function (response) {
            var obj = JSON.parse(response);
            if (obj.status) {
                formattedNumber = obj.rate.replace(/,/g, "");
                $('input[name="wccs_currencies['+code+'][rate]"]').val(formattedNumber);
                button.attr('disabled', false);
            }else{
                console.log('wccs error: problem on updating');
                button.attr('disabled', false);
    ...
    $(document).on("change", "input[name='wccs_update_type']", function(){
        ...
        if ($(this).val() == "api") {
            $(".wccs_update_type_class").removeAttr("required");
        } else if ($(this).val() == "fixed") {
            $(".wccs_update_type_class").attr("required", "required");
        }
    });
    ...
    $(document).on("change", "select[name='wccs_currency_location_type']", function(){
        ShowLocationAPIField($(this).val());
    });
    ShowLocationAPIField($("select[name='wccs_currency_location_type']").val());
...
function ShowLocationAPI(){
    if(jQuery("input[name='wccs_currency_by_location']").is(':checked')){
        jQuery('.wccs_currency_location_type').show();
        if(jQuery("select[name='wccs_currency_location_type']").val() == 'ipapi'){
            jQuery('.wccs_ipapi_key').show();
        }
    } else {
        jQuery('.wccs_ipapi_key').hide();
        jQuery('.wccs_currency_location_type').hide();
    }
}

function ShowLocationAPIField(val){
    if( val == 'ipapi' ){
        jQuery('.wccs_ipapi_key').show();
    } else {
        jQuery('.wccs_ipapi_key').hide();
...
jQuery(document).ready(function($) {
    ...
    function APISelection(value){
        const sections = ['.wccs_exchange_rate_free_api', '.open_exchange_rate', '.abstract_api', '.api_layer_fixer', '.exchange_rate_api'];
