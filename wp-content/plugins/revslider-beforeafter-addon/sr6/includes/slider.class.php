...
class RsBeforeAfterSliderFront extends RevSliderFunctions {
	...
	public function add_addon_files($html, $export){
		...
		$_jsPathMin = file_exists(RS_BEFOREAFTER_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
		if(!$export->usepcl){
			$export->zip->addFile(RS_BEFOREAFTER_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', 'js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js');
			$export->zip->addFile(RS_BEFOREAFTER_PLUGIN_PATH . 'sr6/assets/css/revolution.addon.' . $this->pluginTitle . '.css', 'css/revolution.addon.' . $this->pluginTitle . '.css');

			$export->zip->addFile(RS_PLUGIN_PATH . 'public/css/fonts/font-awesome/css/font-awesome.css', 'css/font-awesome.css');
		}else{
			$export->pclzip->add(RS_BEFOREAFTER_PLUGIN_PATH.'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', PCLZIP_OPT_REMOVE_PATH, RS_BEFOREAFTER_PLUGIN_PATH.'sr6/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
			$export->pclzip->add(RS_BEFOREAFTER_PLUGIN_PATH.'sr6/assets/css/revolution.addon.' . $this->pluginTitle . '.css', PCLZIP_OPT_REMOVE_PATH, RS_BEFOREAFTER_PLUGIN_PATH.'sr6/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');

			$export->pclzip->add(RS_PLUGIN_PATH.'public/css/fonts/font-awesome/css/font-awesome.css', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/css/', PCLZIP_OPT_ADD_PATH, 'css/');
		}

		$html = str_replace(RS_PLUGIN_URL.'public/css/fonts/font-awesome/css/font-awesome.css', 'css/font-awesome.css', $html);
		$html = str_replace(RS_BEFOREAFTER_PLUGIN_URL.'sr6/assets/css/revolution.addon.' . $this->pluginTitle . '.css', 'css/revolution.addon.' . $this->pluginTitle . '.css', $html);
		$html = str_replace(RS_BEFOREAFTER_PLUGIN_URL.'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin .'.js', $export->path_js .'revolution.addon.' . $this->pluginTitle . $_jsPathMin .'.js', $html);
	...
	public function add_scripts() {
		...
		wp_enqueue_style('rs-icon-set-fa-icon-', RS_PLUGIN_URL . 'public/css/fonts/font-awesome/css/font-awesome.css', array(), RS_REVISION);
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['beforeafter'] = '<script'. $addition .' src="'. $this->pluginUrl . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
	...
	public function check_if_ajax_loaded($r, $output) {
		...
		$html = '<link rel="stylesheet" href="'. RS_PLUGIN_URL . 'public/css/fonts/font-awesome/css/font-awesome.css">'."\n";
	...
	public function write_init_script($slider, $id) {
		...
		$settings = $slider->get_params();
		$addOn = $this->get_val($settings, array('addOns', 'revslider-' . $this->pluginTitle . '-addon'), array());
